...
export const allSongs = getAllSongs(library);

function getAllSongs(playlists) {
  let allSongs = [];

  for (const playlistKey in playlists) {
    const playlist = playlists[playlistKey].playlist;
    for (const songKey in playlist) {
      const song = playlist[songKey];
      allSongs.push({
        title: song.title,
        artist: song.artist.join(', '), // Joining the array of artists into a string
        genre: song.genre ? song.genre.join(', ') : '', // Joining the array of genres into a string if present
...
export const artistList = getArtistList(library);
 function getArtistList(library) {
  let artistList = {};

  for (const playlistKey in library) {
    const playlist = library[playlistKey].playlist;
    for (const songKey in playlist) {
      const song = playlist[songKey];
      const artistKey = songKey; // Assuming each song key represents a unique artist
      const artistName = song.artist.join(', '); // Joining the array of artists into a string
      artistList[artistKey] = artistName;
    }
  }
...
export const availableGenres = getAvailableGenres(library);
function getAvailableGenres(library) {
