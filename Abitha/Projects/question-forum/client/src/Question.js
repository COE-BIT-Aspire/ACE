...
const Question = ({ obj, setData }) => {
  ...
  const handleAnswerUpload = (id) => {
    ...
    axios
      .post(`/answer/${id}`, {
        answer: answerRef.current.value.charAt(0).toUpperCase() + answerRef.current.value.slice(1) ,
        answeredBy: nameRef.current.value.charAt(0).toUpperCase() + nameRef.current.value.slice(1),
      })
      .then((res) => {
        setData(res.data);
  ...
  return (
      <div className="question-element">
        ...
        <div className="question-date">
          asked by <div className="asked-by-text">{obj.askedBy}</div> <span></span>
